...
function buildCoverLetterSection(): string[] {
  return [
    ...
    "For cover letters, when no named recipient is available, prefer simple professional salutations such as 'Dear Hiring Team' or 'Dear [Company] team'; avoid stiff Anglo-corporate phrasing when a cleaner option is possible.",
    ...
    "For cover letters, the closing should be short, useful, and contribution-oriented rather than ceremonial, but still end with a proper sign-off block.",
...
function buildCandidatePositioningSection(): string[] {
  return [
    ...
    "For Denmark-local cover letters, prefer a local, non-template opening and use the most specific salutation available; if no contact name is available, use a simple professional fallback rather than omitting the salutation.",
    "For Denmark-local cover letters, keep salutations restrained and low-drama: usually 'Dear Hiring Team' or a specific name if available, not elaborate greetings.",
    "For Denmark-local cover letters, use a practical 3-part flow: work-focused opening, evidence-focused middle, and a short useful closing, while still including a greeting and sign-off.",
    "For Denmark-local cover letters, keep the closing short and useful, with more emphasis on how the candidate can contribute and less on formal courtesy language, but do include a proper sign-off.",
