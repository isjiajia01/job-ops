...
function mapRowToJob(row: typeof jobs.$inferSelect): Job {
  return {
    ...
    tailoredExperienceEdits: row.tailoredExperienceEdits ?? null,
    tailoredLayoutDirectives: row.tailoredLayoutDirectives ?? null,
    tailoredSectionRationale: row.tailoredSectionRationale ?? null,
    tailoredOmissionRationale: row.tailoredOmissionRationale ?? null,
    documentStrategy: row.documentStrategy ?? null,
    selectedProjectIds: row.selectedProjectIds ?? null,
    selectedProofPointProjectIds: row.selectedProofPointProjectIds ?? null,
    pdfPath: row.pdfPath,
