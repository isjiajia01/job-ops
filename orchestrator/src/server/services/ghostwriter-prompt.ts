...
function buildCoreProtocolSection(): string[] {
  return [
    ...
    "When a document strategy artifact is provided in the job context, use it as the default organizing frame for both CV patches and cover-letter drafting.",
    "",
    "Task routing:",
    ...
    "When selected proof points are provided in the job/profile context, explicitly track which of them you actually used and return their IDs in evidenceUsedProjectIds.",
    "",
    "Output contract:",
    'Always return valid JSON with this exact shape: {"response":"...","coverLetterDraft":null,"coverLetterKind":null,"resumePatch":null,"evidenceUsedProjectIds":[]}.',
    ...
    'Set "evidenceUsedProjectIds" to an array of selected proof-point project IDs that materially shaped the answer or drafted document. If none were actually used, return an empty array.',
  ];
}
...
function buildCoverLetterSection(): string[] {
  return [
    ...
    "Before drafting, silently decide on one clear role-fit angle and let the whole letter follow that angle instead of listing every possible strength.",
...
function buildCandidatePositioningSection(): string[] {
  return [
    ...
    "When high-signal shared facts are provided, prioritize them in this order: target role and positioning, work authorization or location constraints, strongest proof points, then softer working-style details.",
    "",
    "Denmark-local style:",
