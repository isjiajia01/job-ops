...
interface ApplicationDetailsEditDrawerProps {
  open: boolean;
...
const emptyDraft: ApplicationDetailsDraft = {
  title: "",
  employer: "",
  jobUrl: "",
  applicationLink: "",
...
const normalizeFromApplication = (
  application: Application | null,
  ...
  return {
    ...
    tracerLinksEnabled: Boolean(application.tracerLinksEnabled),
  };
};
...
export const ApplicationDetailsEditDrawer: React.FC<
  ApplicationDetailsEditDrawerProps
> = ({
  open,
  onOpenChange,
  application,
  ...
  const [draft, setDraft] = useState<ApplicationDetailsDraft>(emptyDraft);
  ...
  useEffect(() => {
    if (!open) return;
    setDraft(normalizeFromApplication(application));
  ...
  const isDirty = useMemo(() => {
    ...
    const current = normalizeFromApplication(application);
    return (
  ...
  const handleSave = async () => {
    ...
    if (
      ...
      !application.tracerLinksEnabled &&
      !tracerCanEnable
    ) {
      setValidationError(
    ...
    try {
      ...
      const employerChanged =
        employer.toLowerCase() !== application.employer.trim().toLowerCase();
      ...
      if (employerChanged) {
        try {
          await api.checkSponsor(application.id);
        } catch (error) {
          const message =
            error instanceof Error
              ? error.message
              : "Application updated, but sponsor check failed";
          toast.error(message);
        }
      }
      ...
      toast.success("Application details updated", {
        action: {
          ...
          onClick: () => {
            void (async () => {
              try {
                await api.rescoreJob(application.id);
                await onApplicationUpdated();
              ...
              <div className="mt-4 flex-1 overflow-y-auto pr-1">
                ...
                <div className="mt-3 rounded-lg border border-border/60 bg-muted/10 px-3 py-3">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">
                      Enable tracer links for this application
                    </span>
                  </label>
                ...
                <div className="mt-3 space-y-1">
                  ...
                  <Textarea
                    ...
                    placeholder="Paste or refine the application description..."
