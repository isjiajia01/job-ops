...
vi.mock("../api", () => ({
  fetchApplicationSourceFromUrl: vi.fn(),
  inferApplicationFromJd: vi.fn(),
  createApplicationFromDraft: vi.fn(),
}));
...
describe("ManualImportSheet", () => {
  ...
  it("runs analyze -> review -> import on the happy path", async () => {
    ...
    vi.mocked(api.inferApplicationFromJd).mockResolvedValue({
      job: {
    ...
    vi.mocked(api.createApplicationFromDraft).mockResolvedValue({
      id: "job-1",
    } as any);
    ...
    await waitFor(() => expect(api.createApplicationFromDraft).toHaveBeenCalled());
    expect(api.createApplicationFromDraft).toHaveBeenCalledWith({
  ...
  it("returns to the paste step when inference fails", async () => {
    ...
    vi.mocked(api.inferApplicationFromJd).mockRejectedValue(
  ...
  it("shows a toast error and keeps the sheet open when import fails", async () => {
    ...
    vi.mocked(api.createApplicationFromDraft).mockRejectedValue(
  ...
  describe("URL fetch functionality", () => {
    ...
    it("fetches URL and proceeds to review on successful fetch", async () => {
      vi.mocked(api.fetchApplicationSourceFromUrl).mockResolvedValue({
      ...
      expect(api.fetchApplicationSourceFromUrl).toHaveBeenCalledWith({
        url: "https://example.com/job",
      });
      expect(api.inferApplicationFromJd).toHaveBeenCalledWith({
    ...
    it("shows error and returns to paste step when fetch fails", async () => {
      vi.mocked(api.fetchApplicationSourceFromUrl).mockRejectedValue(
